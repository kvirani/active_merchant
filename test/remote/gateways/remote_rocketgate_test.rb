...
class RemoteRocketgateTest < Test::Unit::TestCase
 ...
 def test_purchase_and_credit
   @options[:order_id] = 'PURCHASE-CREDIT-' + @options[:order_id]
    assert auth = @gateway.purchase(@amount, @credit_card, @options)
    assert_success auth
    assert_equal 'Transaction Successful', auth.message
    ...
    assert_equal 'Transaction Successful', credit.message
  end
 ...
 def test_purchase_optional_fields
  ...
  def test_card_hash
    @options[:order_id] = 'CARD_HASH-' + @options[:order_id]
    assert response1 = @gateway.purchase(@amount, @credit_card, @options)
    assert_success response1
    assert_equal 'Transaction Successful', response1.message

    @credit_card.number = response1.params['card_hash'];

    assert response2 = @gateway.purchase(@amount, @credit_card, @options)
    assert_success response2
    assert_equal 'Transaction Successful', response2.message
  end
  ...
  def test_unsuccessful_scrub
    @options[:order_id] = 'BAD_SCRUB-' + @options[:order_id]
    @options[:scrub] = true
    assert response = @gateway.purchase(@amount, @credit_card, @options)
    assert_failure response
